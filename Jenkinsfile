pipeline {
    ...
    environment {
        ...
        dockerRegistryCredential='murad-dockerhub'
        dockerImage = ''
        ...
        stage('Deploy') {
            steps {
                script {
                    ...
                    def message = "Deployment of $DOCKER_TAG was successful by Name: Rafiqul Islam Murad and Email: murad.hossain@ba-systems.com"
                    sh """
                    curl -H "Content-Type: application/json" -d '{ "content": "${message}" }' ${DISCORD_WEBHOOK_URL}
                    """
                }
